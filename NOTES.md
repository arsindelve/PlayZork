...
• **The harness mattered as much as the model.** Same qwen, same shadow agent: on Zork's forest it made 1 leaf-goal in 2135 turns; on the escape room it made 4 right-altitude goals with *checkable* closing conditions in 26 — including "find a light source → closes when a light source is in inventory," the exact obstacle. The goal agent wasn't the problem; the game that never presented a puzzle was. Choosing a game with a gradient did more for the goal-creation signal than any prompt tuning.

• **Generation succeeded; pursuit failed — cleanly separated for once.** The shadow agent *named* the right goal (find a light) while the live player walked into the dark room and flailed for 15 turns. Because the goal agent is observe-only, knowing the goal changed nothing. That's the whole argument for active goal-agents in one screenshot: the bottleneck isn't seeing what to do, it's having something that *advocates* it turn after turn. If wiring the goal into the loop doesn't fix the dark room, the thesis is in trouble; if it does, we have our first climb.
