...
# Development Log: 2026-09-10 — Windows/qwen3 bring-up, three agent fixes, and a 25-turn run that pins the generation gap

First session on the Windows box (RTX 5070 Ti, 16GB) after the hardware change.
Local play now runs on `qwen3:14b` via Ollama. Four fixes landed, then a clean
25-turn Zork run on local Ollama to read the priorities.

## Fixes landed

- **`OLLAMA_HOST` normalization** (`c7ce541`). The Ollama Windows app sets
  `OLLAMA_HOST=0.0.0.0:11434` as a User env var; `config.py` handed that
  straight to `ChatOllama` as the connect URL, and every call died with "All
  connection attempts failed" (`0.0.0.0` is a bind address, not a destination,
  and the scheme was missing). `_normalize_ollama_host` maps a bind-all host to
  loopback, adds `http://`, and passes a real remote (incl. bracketed IPv6)
  through untouched.
- **#31 — BLOCKED directions are re-tried, not written off** (`de59216`). The
  explorer counted a BLOCKED edge as explored, so the map repair #11 enabled
  never happened on its own. `explorer_direction_pools` now splits real
  passages (explored) from BLOCKED edges (a low-priority retry pool); a retry
  ranks below all real frontier, and when only a retry remains it is proposed
  at confidence 30 / EV ~1.5. Issue closed.
- **#29 — Observer dedup shows all open issues, undecayed** (`58de1a0`). Option
  3: the duplicate-avoidance list wants *every* open issue (a stale-but-open one
  is exactly what must stay visible), so it is deliberately NOT decayed — that
  would be the #20 reflex and is wrong here. Cap raised 20→50 with a warning
  when it bites. Issue closed. #22 (LoopDetectionAgent) closed won't-fix,
  superseded by #18.
- **Reasoning off by default on Ollama** (`b8a5861`). This is the one that made
  local play usable at all. qwen3 defaults to emitting a `<think>` trace;
  measured on a trivial prompt, **118 output tokens (4.3s) thinking-on vs 2
  tokens (2.2s) with `think:false`**. On the real agent prompts those traces run
  to thousands of tokens, and because five agents fan out concurrently at a
  single Ollama server that serializes them (the 2026-08-24 finding), each
  stacked past the 180s per-attempt timeout — a first 25-turn attempt managed
  **one decision in ~13 minutes**, all five agents timing out at once. Setting
  `reasoning=False` (env-overridable via `PLAYZORK_OLLAMA_REASONING`) dropped
  turns to ~10-11s early / ~32s under full fan-out, zero timeouts, no leaked
  think tags. This project was tuned for the non-reasoning qwen2.5:14b, so OFF
  is the right default.

Full suite 812→819 passing across the four commits; each verified live.

## The 25-turn run (`analysis25b-20260910`, qwen3:14b, multi_agent)

25 turns, **~32s/turn, 0 timeouts, final score 0.** Rooms reached: West of
House → North of House → Forest Path → Clearing → Forest → **Behind House**.
Arbiter overrides (picked below top EV): **8/26**, as M5b predicted the
objective signal would raise; the stagnation-urgency signal fired on turns 10,
12, 21. No BLOCKED retry was offered all run (#10/#33 recorded no false walls in
the opening), so #31 had nothing to correct and did not misfire.

**The decisive finding — the generation gap, caught red-handed.** On turn 18 at
Behind House the arbiter chose **`OPEN WINDOW`** (IssueAgent, "the window is
slightly ajar") — the single action that unlocks the whole game, since the open
window leads to the Kitchen and Zork's first points. Turn 19 then chose
**`NORTH`**: it walked away. Across all 25 turns **no agent ever proposed
`ENTER WINDOW` / `IN` / `CLIMB THROUGH`** (grep: zero hits). The system did the
hard part — navigate to the window and open it — and could not take the trivial
follow-through, because nothing generates it:

- the IssueAgent's issue was "examine or open the window" — once opened, done;
- the InteractionAgent proposes *object* verbs from the backend's accepted-command
  list (open/close/examine); *entering* is a movement, so it structurally cannot
  propose it;
- the ExplorerAgent only proposes cardinal frontier.

This is direct evidence for M5b's stated limitation: the arbiter can only rank
what it is given. **B (strengthen the arbiter) is confirmed insufficient — the
gap is generation, not selection.** Score stayed 0 not because the agent never
found the opening but because nothing advocated walking through it.

## What this argues for — the finding, stated generally

The window is one instance, not the problem. The arbiter ranks a CANDIDATE SET,
and that set is generated by three reactive, narrow sources: IssueAgent (flagged
issues), ExplorerAgent (cardinal directions), InteractionAgent (object verbs the
backend lists for the *current* room). Their union is a strict SUBSET of the
useful action space. Whenever the right next move falls outside all three
vocabularies — enter the window, walk back three rooms to the thing you left,
put the coffin in the boat, wait for the troll to leave — nothing proposes it
and the arbiter cannot choose it. A rule like "open X then enter X" patches one
symptom; the defect is that candidate GENERATION is too narrow.

Two compounding causes:

1. **No generator reasons from the objective.** `GAME_OBJECTIVE` is a prompt
   header that drives no proposal. Every proposer reacts to what is present or
   already flagged; none asks "given the goal and all that is known, what should
   I try that nobody is tracking yet?"
2. **Issues are leaves, not goals.** "Open the window" resolved the instant the
   window opened. Nothing represents a multi-step objective ("get inside the
   house") that persists and re-emits its next concrete step until a goal state
   is reached. The system has flags, not plans, so it cannot sequence.

**The thesis-level conclusion (not a bug):** advocacy + arbitration measurably
improved SELECTION — M5b overrides fire, the arbiter reasons in objective terms
— but GENERATION is the binding constraint on whether the architecture can play
a long-horizon game at all. Selection quality cannot exceed what generation puts
on the ballot, and the audit spent itself improving selection. See NOTES.md
2026-09-10 for the implications.

Two real directions, neither a surgical patch:

- **A generative proposer** (PLAN.md's deferred arm A): reads objective + full
  context and generates candidates the reactive agents cannot. The direct answer
  to "the gap is generation." The named risk is real — if it wins most turns the
  architecture has collapsed into a single-shot player and the comparison is
  confounded — but that is a measurement problem to instrument, not a reason the
  generator is wrong.
- **Stateful, decomposing issues:** reframe memory from flags to
  goals-with-progress, where an issue carries a target state and re-emits its
  next step each turn until reached. Attacks cause #2 directly and stays inside
  the advocacy/arbiter frame, but is a deeper change to the memory model and the
  observer/closer contract.

Lower-priority, still real:

- **Tune the urgency override.** Turn 23 chose `EAST` at EV 7.0 over a proposal
  at EV 70.0 — the stagnation signal overrode a strong proposal with near-noise.
  Wants a floor or a smaller multiplier.
- The open infra issues (#24 summarizer latency, #27 single-server concurrency,
  #32 inventory resync) are **less urgent for local dev now** — reasoning-off
  removed the latency crisis on this hardware. They matter again for a
  throughput-sensitive experiment protocol, not day-to-day play.
