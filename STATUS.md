...
# Development Log: 2026-09-10 — Windows/qwen3 bring-up, three agent fixes, and a 25-turn run: the window the agent opened, then abandoned
...
**The decisive finding — the window the agent opened, then abandoned.** Traced
turn by turn from the log:

- **Turn 18**, at Behind House, the arbiter chose **`OPEN WINDOW`** (IssueAgent
  #1, the window issue ID:19). The game replied: *"With great effort, you open
  the window far enough to allow entry."* — the way into the Kitchen and Zork's
  first points.
- **Turn 19**, at Behind House with the window now open, the entire ballot was:
  InteractionAgent `nothing` (conf 0); ExplorerAgent `NORTH` (75); IssueAgent
  ID:19 **`CLOSE WINDOW`** (20); IssueAgent ID:16 `EAST` (70). The arbiter chose
  `NORTH`. **No agent proposed entering** — no `IN` / `ENTER` / `WEST` / `CLIMB
  THROUGH` appears anywhere in the 25 turns (grep: zero hits).

Two log lines are the whole story:

- The **IssueClosedAgent** staged the window issue closed, quoting the game
  verbatim: *"ID 19 … was opened, as shown in the game history: 'you open the
  window far enough to allow entry.'"* — it used the sentence announcing entry
  is now possible as its proof the issue was **done**.
- The window's own **IssueAgent proposed `CLOSE WINDOW`**, reasoning *"the window
  is currently open enough to allow entry, but the issue is to examine or open
  it. Since it is already open, closing it may be a step toward further
  interaction."* — it advocated shutting the window it had just opened.

Why: issue ID:19 was authored (by the ObserverAgent) as **"Small window at
Behind House — examine or open it"** — the *action*, not the *aim*. The instant
the window opened, that acceptance criterion was met, so the closer retired the
issue and the still-running IssueAgent, bound to the same finished text, flailed
to `CLOSE WINDOW`. No issue and no agent ever represented "get inside the house,"
so once the window was open, nothing pointed through it.

This is direct evidence for M5b's stated limitation: the arbiter can only choose
among what the drives advocate. The agent did the hard part — navigate to the
window and open it — and could not take the one-step follow-through, because no
drive in the roster was aimed through the opening. Score stayed 0 not because
the opening was never found, but because no agent's drive wanted to go through
it.

## What the chain reveals (findings, not fixes)

Stated in the architecture's own terms — the agents *are* the competing drives
(ExplorerAgent = explore, IssueAgent = resolve a flagged obstacle, Interaction­Agent
= engage what is in the room), and the arbiter chooses among what they advocate:

1. **The roster of drives is a start, not a complete set.** Across 25 turns the
   union of {explore, resolve-issues, interact-here} never once advocated "get
   inside the house / go through the opening." A move no drive is aimed at is
   never on the ballot, however good the arbiter is. This is not a challenge to
   the deliberation thesis — completing and refining the roster of drives *is*
   the architecture under test, and having it be adequate is a precondition for
   the comparison to be interpretable, in the same sense state-correctness was.

2. **Issues are authored as leaf actions, not aims.** The ObserverAgent writes
   the issue as the thing to *do* ("examine or open the window"), so the one
   drive with momentum toward the house terminated — and then actively reversed
   (`CLOSE WINDOW`) — at the exact moment its action completed, which is exactly
   when the payoff was one step away. The IssueClosedAgent behaved correctly
   given the criterion; the criterion was the wrong shape (a means, not an end).

Whether the missing behaviour is a **new drive** in the roster, or an **existing
...
- **The stagnation-urgency signal can override a strong proposal with
  near-noise:** turn 23 chose `EAST` at EV 7.0 over a proposal at EV 70.0.
